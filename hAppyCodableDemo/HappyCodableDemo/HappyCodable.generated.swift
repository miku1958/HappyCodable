...
//
//  Created by 庄黛淳华 on 2020-08-01 14:20:55 +0000.
...
extension CodingKeysExistStruct {
	...
	func decode(happyFrom decoder: Decoder) throws {
		...
		do { self.int = try container.decode(default: self.int, key: "int_alter", alterKeys: []) } catch { errors.append(error) }
		do { self.string = try container.decode(default: self.string, key: "string_alter", alterKeys: []) } catch { errors.append(error) }
		do { self.bool = try container.decode(default: self.bool, key: "bool", alterKeys: []) } catch { errors.append(error) }
